...
export const createItemWithAuth = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const itemId = await ctx.runMutation(components.myComponent.public.createItem, {
...
```typescript
// src/client/index.ts
import { FunctionReference, GenericMutationCtx, GenericQueryCtx } from "convex/server";
