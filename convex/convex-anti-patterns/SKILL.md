...
export const createOrder = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const price = await fetch(`https://api.stripe.com/prices/${args.productId}`);
    await ctx.db.insert("orders", { productId: args.productId, price: await price.json() });
    return null;
  },
});
```
...
export const fetchPrice = internalAction({
  ...
  returns: v.null(),
  handler: async (ctx, args) => {
    const order = await ctx.runQuery(internal.orders.getById, { orderId: args.orderId });
    if (!order) return null;

    const response = await fetch(`https://api.stripe.com/prices/${order.productId}`);
...
export const getPostsWithAuthors = query({
  ...
  handler: async (ctx) => {
    ...
    const authorMap = new Map(
      authors.filter((a): a is NonNullable<typeof a> => a !== null)
        .map((a) => [a._id, a])
    );
...
export const transferFunds = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.from, { balance: fromAccount.balance - args.amount });
  ...
  handler: async (ctx, args) => {
    try {
      ...
      if (args.attempt < 5) {
        ...
        await ctx.scheduler.runAfter(baseDelay + jitter, internal.jobs.processWithRetry, {
          jobId: args.jobId,
          attempt: args.attempt + 1,
        });
      }
