...
export const sendEmail = internalAction({
  ...
  returns: v.object({ success: v.boolean(), messageId: v.optional(v.string()) }),
  handler: async (ctx, args) => {
...
export const sendExpiringMessage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(args.expiresInMs, internal.messages.delete, { id });
      ...
      const stripeResponse = await fetch(
        "https://api.stripe.com/v1/subscriptions",
        {
          ...
          body: new URLSearchParams({
            ...
            items: [{ price: args.planId }].map((i, idx) => `items[${idx}][price]=${i.price}`).join("&"),
          }),
        }
      );
...
const jitter = Math.random() * 1000;
await ctx.scheduler.runAfter(baseDelay + jitter, internal.jobs.retry, { jobId });
```
