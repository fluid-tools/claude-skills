...
async function getUserName(ctx: QueryCtx, userId: Id<"users">): Promise<string | null> {
...
export default defineSchema({
  notifications: defineTable({
    userId: v.id("users"),
    type: v.union(v.literal("message"), v.literal("mention"), v.literal("system")),
    // Common fields
    read: v.boolean(),
    createdAt: v.number(),
    ...
    data: v.union(
      ...
      v.object({ type: v.literal("mention"), messageId: v.id("messages"), mentionedBy: v.id("users") }),
      v.object({ type: v.literal("system"), title: v.string(), body: v.string() })
    ),
  }).index("by_user", ["userId"]),
});
```
